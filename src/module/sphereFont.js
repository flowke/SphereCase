...
var radius = window.innerHeight/4+20;
var dtr = Math.PI/180;
var d=500;
...
window.addEventListener( 'resize', ()=>{
    radius = window.innerHeight/4+20;
    if(radius<90){
        oDiv.style.opacity = 0;
        return
    }
...
function update()
{
	...
	for(var j=0;j<mcList.length;j++)
	{
		...
		mcList[j].y=ry3*per;
		mcList[j].scale=per/10;
		mcList[j].alpha=per;

		mcList[j].alpha=(mcList[j].alpha-0.8)*(10/6);
	}
...
function doPosition()
{
	...
	for(var i=0;i<mcList.length;i++)
	{
		...
		aA[i].style.fontSize=Math.ceil(9*mcList[i].scale/9)+'px';

		aA[i].style.filter="alpha(opacity="+100*mcList[i].alpha+")";
		aA[i].style.opacity=mcList[i].alpha/1.5;
	}
}
